...
Relevance (Evidence lens) and perspective engagement (Scope lens) are the two Core-tier facets with the highest cross-lens visibility:
...
High cross-lens visibility makes these facets ideal for early sessions, where the Diagnose → Discuss cycle needs to work well for students to see the value of different lenses. They're also the most intuitive facets — students readily grasp "the evidence doesn't match" and "they ignored a good point" or "they only considered their own perspective" even before developing formal vocabulary.
...
- **Breadth across Core tier:** Scenarios 2 and 3 introduce the remaining three Core-tier facets (sufficiency, source credibility, inferential validity), ensuring all five are covered.
- **Extend-tier facets as companions:** When anchors return in Scenarios 4 and 5, they're paired with Extend-tier facets (condition sensitivity, consequence consideration). This models the natural progression — students who have practiced noticing evidence-claim mismatch are ready to notice boundary conditions; students who have practiced noticing dismissed arguments and missing perspectives are ready to notice missing consequences.
...
Scenario 2: New Core-tier facets (accessible)
Scenario 3: New Core-tier facets (moderate — inferential validity has low cross-lens visibility)
Scenario 4: Anchor return + Extend-tier (subtler relevance signal)
Scenario 5: Anchor return + Extend-tier (subtler perspective-engagement signal)
```
...
2. **Cross-lens visibility** — Scenario 3 includes inferential validity, which has the lowest cross-lens visibility of any Core-tier facet (Logic-specific, rarely visible through other lenses). This is the hardest observation task in the sequence.

---

## Sequence Overview

| # | Scenario | Facets | Productive Lenses | Cognitive Patterns | Social Dynamics | Difficulty |
|---|---|---|---|---|---|---|
| 1 | Ocean vs Deforestation | **relevance** + **perspective_engagement** | Evidence, Scope, Logic | overgeneralization, confirmation_bias | conflict_avoidance, group_pressure | Accessible |
| 2 | School Garden Water | sufficiency + perspective_engagement | Evidence, Scope, Logic | false_certainty, egocentric_thinking | authority_deference | Accessible |
| 3 | Recycling Program | source_credibility + inferential_validity | Evidence, Logic | uncritical_acceptance, false_cause | authority_deference | Moderate |
| 4 | Climate Poster Claims | **relevance** + condition_sensitivity | Evidence, Scope, Logic | confirmation_bias, overgeneralization | group_pressure | Moderate–Challenging |
| 5 | Field Trip Proposal | **perspective_engagement** + consequence_consideration | Scope, Logic | tunnel_vision, black_and_white_thinking | conflict_avoidance | Moderate–Challenging |

**Note on lens column:** "Productive Lenses" lists the lenses through which a student can make meaningful observations in this scenario, including cross-lens routes. Students choose their own lenses — they are not assigned. The targeted facets determine the primary lenses; cross-lens visibility determines which additional lenses are productive.

**Coverage:**
- Core-tier facets: 5/5 (all covered)
- Extend-tier facets: 2/4 (condition_sensitivity, consequence_consideration)
- Extend-tier not covered: source_diversity, reasoning_completeness (see Coverage Gaps below)
- Reserve-tier: internal_consistency (intentionally excluded — hardest to design for, per facet inventory)
- Cognitive patterns: 8/8 (all covered)
- Social dynamics: 3/3 (all covered; distributed to avoid over-reliance on any single dynamic)
...
- **Strength signal:** The deforestation persona's feasibility argument ("we don't live near the ocean," "can we actually do anything about this?") is genuinely strong reasoning — practical, specific, well-grounded. Students should be able to identify this as a moment of sound reasoning through the Scope lens, even though the group ultimately fails to engage with it. The contrast between the strong argument and the group's failure to engage it is what makes the perspective-engagement weakness visible.
- **Target passages:** 2 passages. First passage targets relevance (the documentary evidence stretch). Second passage targets perspective engagement (the feasibility dismissal). The strength signal lives in the second passage.
...
- Perspective engagement (Scope lens) — neither persona considers the custodial
  ...
  perspective as students). Social dynamic: authority deference (once the
  hose persona deferred to the "research," both personas treated the
  decision as settled and shifted into execution mode — the deference that
...
- Check that authority deference is distinguishable from general social pressure — the hose persona defers specifically because the other "did the research," not because of enthusiasm or social momentum.
- **Strength signal:** The hose persona's initial pragmatic reasoning — the school hose is simple, reliable, and guaranteed to work by the principal's spring deadline — is genuinely sound. Students should recognize this as strong reasoning through the Evidence lens (practical evidence) before it gets abandoned. The contrast between this sound practical reasoning and the thin evidence that replaces it is what makes the sufficiency weakness visible.
- **Target passages:** 2 passages. First passage targets sufficiency (the Portland website extrapolation and authority deference). Second passage targets perspective engagement (the excited planning that excludes all other stakeholders). The strength signal lives in the first passage (the hose persona's practical reasoning before deference).
...
- Inferential validity (Logic lens) — the same persona argues: "the
  ...
  authority deference (the worried persona's sibling claim gets
  outweighed by the other persona's "actual data" — the appearance of
...
- **Strength signal:** The worried persona's initial position — raising the sibling's claim about single-stream ending up in landfills — is genuinely strong reasoning. They're bringing a counter-claim, citing a different source, and expressing appropriate skepticism. Students should recognize this as strong Evidence reasoning (source diversity, healthy skepticism) before it collapses under authority deference. The contrast between "I heard something different" (strong) and "well, you found actual data" (deference to appearance of evidence) is powerful.
- **Target passages:** 2 passages. First passage targets source credibility (the recycling company website and the worried persona's deference to "actual data"). Second passage targets inferential validity (the flawed inference combining staff difficulty with the company's claim). The strength signal lives in the first passage (the worried persona's initial skepticism).
...
- **Strength signal:** The accuracy-minded persona's initial push for precision — questioning whether "recycling saves the planet" is really what the aluminum statistic shows — is genuinely strong reasoning. This is sound Evidence evaluation (noticing the gap between specific evidence and a broad claim). Students should recognize this moment of strong reasoning through the Evidence lens before it gets overwhelmed by poster-design excitement. The contrast between careful questioning and eventual capitulation is what makes both the relevance and condition-sensitivity weaknesses visible.
- **Target passages:** 2 passages. First passage targets relevance (the aluminum-to-planet stretch). Second passage targets condition sensitivity (unqualified poster claims). The strength signal lives in the first passage (the accuracy persona's initial questioning).
...
- Consequence consideration (Scope lens) — the group picks the nature
  ...
  dynamic: conflict avoidance (once they reach agreement, neither
  persona wants to reopen the discussion by raising complications —
  the friendly resolution feels like success, and questioning the
...
- **Strength signal:** The quality of the personas' engagement with *each other* is genuinely strong. They listen, respond to specific points, and make concessions — this is sound reasoning about each other's arguments. Students should recognize this through the Scope lens (or Evidence lens) as a moment of genuine strength. The insight is that strong interpersonal engagement does not guarantee a well-considered decision — the weakness is structural (what categories of consideration are absent), not interpersonal.
- **Target passages:** 2 passages. First passage targets perspective engagement (the well-argued but narrowly-scoped comparison of trip experiences). Second passage targets consequence consideration (the decision made without downstream thinking). The strength signal is distributed — the quality of engagement is visible across both passages, which is what makes the structural weakness harder to detect.
...
| Facet | Priority Tier | Scenarios | Notes |
|---|---|---|---|
| relevance | Core | 1, **4** | Anchor — returns at higher subtlety |
| perspective_engagement | Core | 1, 2, **5** | Anchor — covers dismissed arguments, missing stakeholders, and structural absence |
| sufficiency | Core | 2 | |
...
Three facets from the inventory are not targeted in this 5-scenario sequence:

| Facet | Priority Tier | Why Excluded |
|---|---|---|
| source_diversity | Extend | Partially covered through sufficiency (Scenario 2) — thin evidence from a single source touches both facets. Introducing source diversity as a distinct target requires a scenario where there are *multiple* sources that trace to the same origin, which is a more sophisticated signal than single-source reliance. Best introduced after students are comfortable identifying sufficiency problems. Candidate for a 6th scenario. |
| reasoning_completeness | Extend | The "missing steps" observation is abstract for 6th graders and overlaps with how students naturally perceive sufficiency ("they jumped to a conclusion") and inferential validity ("the conclusion doesn't follow"). Introducing it as distinct from these requires careful calibration. Candidate for a 6th scenario or a later pilot phase. |
| internal_consistency | Reserve | Hardest to design for — contradictions in a 10–14 turn, 2-persona discussion tend to feel planted rather than natural. Intentionally deferred per the facet inventory's Reserve tier designation. |
...
All 3 social dynamics covered. Group pressure appears in 2 scenarios (down from all 5 in the original design) to ensure students learn to distinguish it from other dynamics. Conflict avoidance and authority deference each appear twice.

**Design rationale for social dynamic distribution:** Group pressure is the most common dynamic in real middle school groups, but if it appears in every scenario, students learn to name it reflexively rather than observing it specifically. Scenarios 2, 3, and 5 now have more specific dynamics as their primary social force — authority deference in 2 and 3 (where deference to "research" or "data" is the mechanism), conflict avoidance in 5 (where friendly compromise papers over structural gaps). Group pressure is reserved for scenarios where collective momentum (not one-on-one deference) is the operative mechanism.

### Lens Coverage

| Primary Targeting Lenses | Scenarios |
|---|---|
| Evidence + Scope | 1, 2, 4 |
| Evidence + Logic | 3 |
| Scope + Scope | 5 |

Evidence appears as a primary targeting lens in 4 scenarios, Scope in 4, Logic in 1. However, Logic is a productive lens through cross-lens visibility in 4 of 5 scenarios (all except Scenario 2, where the cross-lens route is weaker). See "Why Logic Appears as Primary Lens in Only One Scenario" in the Rationale for why this is intentional.
