...
- **Architecture checkpoint** — the operator reads the named artifacts against the twelve rules in memo §6 and the three-affordance-at-three-scales matrix in memo §1.2. Expensive relative to mechanical gates; the plan places one at each stage boundary where a silent Rule 11 or Rule 12 violation is plausible. Spec §5 names architecture reviews after Stages 4 and 6; this plan keeps both and adds checkpoints at Stage A exit (archive classification), Stage B exit (inventory/graph), and Stage C exit (schema reality check) because each is a structural commit point whose downstream cost of error is large.
...
- `framework/docs/pipeline-revision-inventory.md` §"Spike resolution — turn annotation granularity" — the calibration of the resolved policy (spec §2.1, memo §3.1) against Overton Park episode 3's densest passage. Records: the hand-annotation, token count, load-bearing vs. empty entry counts, and the borderline-turn list with per-case resolutions.
...
5. Build the dependency graph. The expected shape: schema items precede prompt items for the same agent; the analyst prompt item precedes the diagnostic, prose, and discussion prompt items (diagnostic/prose/discussion all read analyst output from file); the merge script precedes the package reviewer (the reviewer needs a merged package to review). Validate that the graph is acyclic by running a topological sort — either a short Python script that parses the adjacency list from the markdown and calls `graphlib.TopologicalSorter`, or a hand-computed topological order recorded in the inventory. Cycle → stop and restructure.
6. Execute the turn-annotation granularity spike as **policy calibration** (not policy selection — the policy is already resolved in spec §2.1 and memo §3.1). Hand-annotate the densest passage of Overton Park episode 3 under the resolved policy: every turn gets an entry with `speaker` and `turn_id`; content fields populated iff the turn is load-bearing in the framework-relative sense (facet signal, lens transition, cognitive/social dynamic, or claim anchoring later framework-visible content). Record: (a) total token count of the serialized YAML; (b) count of load-bearing vs. empty entries; (c) a list of borderline turns where the load-bearing decision was ambiguous, with the operator's chosen resolution and a one-sentence rationale per case. The borderline list becomes worked-example material for the analyst prompt in Stage D.1. Write the calibration report into the inventory's spike-resolution section.
...
- **B.G3 (mechanical).** The spike-resolution section contains the calibration report (token count, load-bearing vs. empty counts, borderline-turn list with per-case resolutions). The resolved policy from spec §2.1 and memo §3.1 is recorded as the acceptance rule for the `turn_annotations` inventory item; the borderline-turn list is earmarked as worked-example material for the Stage D.1 analyst prompt.
...
5. Commit the schemas and the gold files in the same commit. The commit message names Overton Park episode 3 as the gold baseline. **Matrix-coverage authoring constraint (spec §2.1, §2.3, memo §3.1, §3.2):** the gold `ground_truth.yaml` and `diagnostic.yaml` must together exercise at least four distinct cells of the engagement/affordance matrix, with the non-negotiable cells being `(engagement: none, affordance: rich)` (the highest-urgency blindspot case) and at least one cell with `engagement ∈ {partial, high}` (the fully-populated categories case). Additional recommended cells: `(engagement: none, affordance: thin)` (low-urgency habit-building) and `(engagement: partial, affordance: rich)` (the case where all four categories populate with the partial_readings cell non-empty). If Overton Park episode 3 does not naturally contain passages exercising these cells when the analyst surveys its `lens_visibility`, the operator either (a) selects a different episode that does, updating the baseline declaration in this task and in Stage D, or (b) records the gap in the inventory and authors additional minimal passages from other stories to cover the missing cells. This constraint ensures the gold exercises the matrix breadth so D.2.G1's mechanical checks have coverage to validate against, and so Stage D's agent prompts are not tuned against a single cell.
...
- **C.G3 (mechanical — bootstrap).** `python3 framework/pipeline/initialize.py` succeeds on the post-task-6 tree. `.claude/commands/` contains every file currently in `framework/pipeline/commands/` — the gate checks set equality against whatever that directory holds at Stage C exit, not against a hardcoded list, because Stage E will add `build_assistive_package.md` and the gate would otherwise spuriously fail if E authoring has begun in parallel. For reference, the expected set at Stage C exit is `brainstorm.md`, `create_episode.md`, `create_transcript.md`, and the legacy `analyze_transcript.md`; the legacy `/analyze_transcript` command (and its `evaluator` / `analysis_reviewer` agents) remains live in `framework/pipeline/` through Stages C, D, and E — it is retired from the live tree only in F.2 task 1. `.claude/agents/` contains every file from `framework/pipeline/agents/` (set equality, same reasoning). No files are synced from anywhere under `apps/`. Exit code 0. No errors.
- **C.G4 (architecture checkpoint — schema reality check, from spec §5 Stage 1 gate review).** Operator reads the gold files against memo §1.2. Every cell of the 3×3 affordance-at-three-scales matrix is exercised by at least one field in the gold files. Every field in the gold files traces to one cell. Fields that cannot be placed in the matrix are flagged as candidates for removal or as evidence the matrix is incomplete (the latter is a memo-edit, not a plan-edit). Findings are written as a short report appended to the inventory; schema and spec are revised if necessary.
    ...
    **Absence thought experiment (anti-overfit sub-check).** After the matrix-coverage check passes, the operator walks every required field in every schema and asks: *"If a hypothetical thin or sparse story had nothing real to put in this field, would the schema still be valid?"* Each field lands in one of three categories:
    1. **Can be legitimately empty** — change the schema from "required, non-empty" to "required, may be empty" (or "optional"). Memo §6 Rule 3 ("light usage is valid") is the authority.
    2. **Absence would indicate a story that shouldn't exist** — the field stays required. Document *why* in a schema comment citing the specific affordance the field realizes. This makes the design commitment explicit rather than accidental.
    3. **Absence only makes sense when a specific creative choice is off** — the field belongs behind a capability flag per Rule 2, not in the default schema. Move it. If no second story yet justifies the flag under Rule 2's "≥2 distinct stories using it differently" test, document the field as a known gap in the inventory and defer until a second story is available.

    Rationale: the matrix-coverage check proves *completeness* (the schemas cover the design space). The absence thought experiment proves *robustness* (the schemas do not silently assume Overton Park's richness). A schema can be complete and still overfit; this sub-check catches overfit at the cheapest possible point, before any agent prompt is written. C.G4 exits with two artifacts: the matrix-coverage report and the absence-thought-experiment report listing every required field and its category. Fields landing in category 1 or 3 trigger schema edits before C exits.
...
- **One-off exception for D.1:** the analyst prompt is ported from `framework/pipeline/agents/evaluator.md`, so D.1 (and only D.1) may read that file as source material. No other substage reads it.
...
**Tasks.**
1. Port `framework/pipeline/agents/evaluator.md` into `analyst_agent.md`. Strip every instance of pedagogical speculation (any field the old evaluator wrote that is now a diagnostic-, prose-, or discussion-agent concern). Every stripped field is logged in the diff report. Declare an *optional* read-only input: sibling episodes' prior `ground_truth.yaml` files under `artifacts/{story_id}/episodes/episode_{MM}/` for any `MM < NN`, used to resolve `connects_to` references against concrete prior content. Absence is valid (first-episode runs); presence must not be ignored.
...
- **D.1.G2 — Half A (mechanical; correctness floor; from spec §5 Stage 2 exit criterion; BLOCKING).** ≥90% of hand-authored `facets_present` entries are produced by the analyst with matching `facet_ref` and at least one overlapping `evidence_turn` (intersection non-empty — partial overlap counts as a match; perfect set equality is not required). 100% of analyst turn citations resolve to real turns in the transcript. Every `causal_layer` entry has a populated enumerated `interaction` field. `causal_layer_episode` present. `discussion_cue_seeds[]` populated on every load-bearing turn (per the Stage B spike decision). Half A failures are unambiguously the agent's fault (schema-invalid output, citations that don't resolve, missing required blocks) and trigger prompt tuning per the rollback rule.
- **D.1.G2 — Half B (informational delta; anti-overfit; NON-BLOCKING).** The operator reads every analyst finding that is *not* in the gold (extensions beyond the gold) and classifies each as: **legitimate extension** (real facet the gold missed, well-evidenced in the transcript), **hallucination** (not supported by the transcript), or **ambiguous**. The classification is appended to the inventory as a diff report. Half B does not block substage exit. Its purpose is to distinguish "agent is wrong" from "agent disagrees with the gold" — if Half B surfaces many legitimate extensions, the gold is under-authored and Stage C is regressing, not the agent. An operator may **never** tighten the analyst prompt in response to legitimate extensions; doing so trains the agent to mimic the gold, which is the overfit direction. The escalation rule in D.1 rollback measures improvement on Half A metrics only; improvements that come from suppressing legitimate extensions do not count.
- **D.1.G3 (agent review — analyst fidelity).** An ad-hoc review pass (operator or a dedicated reviewer subagent if cheap) reads the generated file against the gold and reports where the analyst over- or under-reached. Findings feed the Half B classification.

**Rollback.** If D.1.G2 Half A fails, revise the analyst prompt and rerun. Escalation rule: if two successive prompt-tuning rounds produce less than 5% improvement on the failing Half A metric (match rate, citation resolution, schema validity), stop tuning and escalate. Improvements that come from suppressing Half B legitimate extensions do not count as improvement. If Half B surfaces many legitimate extensions, treat it as a Stage C gold regression (the gold is thin), not a D failure — return to C and re-author the affected gold sections in a fresh context per the authorship discipline — the problem is structural, not prompt-level. Escalation means: re-examine whether the analyst's cognitive job is correctly scoped (a memo §3.1 question) or whether `ground_truth.schema.yaml`'s fields are correctly defined (a spec §2.1 question). If memo §3.1 or spec §2.1 must change, follow the spec-edit cascade rules in §III.3.
...
**Gates.**
- **D.2.G1 (mechanical).** `validate_schema.py` passes. `next_move` is a non-empty string on every `response_space` entry. `recommended_lens_switch` populated on every `blindspots[]` entry. **Engagement/affordance matrix enforced (spec §2.1, §2.3, §2.6):** for every lens in the generated diagnostic, read `ground_truth.lens_visibility.{lens}.(engagement, affordance)` and verify: (a) `(partial, none)` and `(high, none)` are rejected as invalid; (b) `likely_readings[]` non-empty iff `engagement ∈ {partial, high}` AND `affordance ≠ none`; (c) `partial_readings[]` non-empty iff `engagement: partial` AND `affordance ≠ none`; (d) `misreadings[]` non-empty iff `engagement ∈ {partial, high}` AND `affordance ≠ none`; (e) `blindspots[]` non-empty iff `affordance ∈ {moderate, rich}` OR (`affordance: thin` AND `engagement ∈ {none, partial}`); (f) all four categories empty iff `affordance: none`; (g) every `blindspots[]` entry carries `text`, `next_move`, `recommended_lens_switch`, `description`, and `why_predictable`.
- **D.2.G2 — Half A (agent review — diagnostic specificity; correctness floor; from spec §5 Stage 3 gate review; BLOCKING).** Sample five entries per category (likely_readings, partial_readings, misreadings, blindspots) across the three lenses. Mark each as "passage-specific" (names something in the actual transcript) or "generic" (could apply to any passage). ≥80% passage-specific to pass. Generic entries are unambiguously the agent's fault. **Blindspot specificity calibrated by matrix cell** (memo §3.5 criterion 5): `(engagement: none, affordance: rich)` blindspots are held to the highest bar — a generic entry in this cell counts as a larger failure than generic content in any other cell, because it is the passage's most pedagogically urgent content. `(engagement: none, affordance: thin)` blindspots may be more general (habit-building framing). The sample must include at least two `(engagement: none, affordance: rich)` blindspots when the gold contains any.
- **D.2.G2 — Half B (informational delta; anti-overfit; NON-BLOCKING).** The operator reads diagnostic entries the gold did not anticipate (new misreadings, new blindspots, new partial readings that don't map to the gold's categories) and classifies each as **legitimate extension** (a real student reading the gold missed), **over-reach** (not a plausible student reading), or **ambiguous**. Appended to the inventory. Prompt tuning may *not* suppress legitimate extensions. If Half B surfaces many legitimate extensions, the gold's student error model is thin and is the C regression, not the agent.

**Rollback.** If D.2.G2 Half A fails, the diagnostic prompt is under-constrained; add counterexamples from Overton Park ep 3 and re-run. Same escalation rule as D.1: two tuning rounds with <5% improvement on Half A triggers escalation; improvements from suppressing Half B legitimate extensions do not count. The problem may be upstream (analyst output too thin to support diagnostic specificity — D.1 needs revisiting), or it may be structural (diagnostic.schema.yaml under-specifies what "passage-specific" means — a spec §2.2 question). Follow §III.3 spec-edit cascade rules if spec or memo must change.

#### D.3 — Prose agent (spec §5 Stage 4)

**Tasks.**
1. Write `prose_agent.md` from scratch. Declare the story design doc as an explicit read-only input (spec §5 Stage 4). The agent reads `ground_truth.yaml` and `diagnostic.yaml` from file. Declare an *optional* read-only input: sibling episodes' prior `prose.yaml` files under `artifacts/{story_id}/episodes/episode_{MM}/` for any `MM < NN`, used to check `pedagogical_register` drift across episodes. Absence is valid.
...
- **D.3.G2 — Half A (agent review — pedagogical register; correctness floor; from spec §5 Stage 4 gate review; BLOCKING).** Sample five entries per block. Mark each as "student-sounding" or "adult-sounding." ≥80% student-sounding to pass. Confirm register matches the story's declared `pedagogical_register`. Adult-sounding prose is unambiguously the agent's fault.
- **D.3.G2 — Half B (informational delta; anti-overfit; NON-BLOCKING).** The operator reads prose entries that diverge from the gold in voice or framing and classifies each as **legitimate variation** (different wording, still student-sounding and register-matched), **drift** (wrong register, wrong voice), or **ambiguous**. Appended to the inventory. Prompt tuning may *not* suppress legitimate variation — doing so trains the prose agent to mimic one author's voice rather than honor the story's register. If Half B surfaces many legitimate variations, the gold's prose is one valid realization among many, not the definition of correct.
...
- **D.4.G2 — Half A (agent review — creative generativity; correctness floor; from spec §5 Stage 5 gate review; BLOCKING).** Sample ten cues. Mark each as "generative" (introduces a new angle or framing) or "paraphrase" (rewords ground truth). ≥70% generative to pass. Confirm at least two of the three axes are exercised across the episode. Confirm persona-projected cues honor established character voices. Paraphrase cues and mis-voiced persona cues are unambiguously the agent's fault.
- **D.4.G2 — Half B (informational delta; anti-overfit; NON-BLOCKING).** The operator reads cues that exercise the three axes in ways the gold did not (new lens refractions, new persona projections, new stance inversions) and classifies each as **legitimate extension** (the axis is honored with a novel angle), **template convergence** (different wording but the same underlying angle as a gold cue — a weak form of Rule 12 violation), or **ambiguous**. Appended to the inventory. Prompt tuning may *not* suppress legitimate extensions. Legitimate extensions here are especially valuable: they are evidence that the three-axis machinery is generative rather than Overton-Park-shaped.

**Rollback.** If D.4.G2 Half A fails, the discussion prompt's three-axis structure is under-specified; add worked examples of each axis from the gold discussion.yaml and re-run. Same escalation rule as D.1–D.3: two tuning rounds with <5% improvement on Half A triggers escalation; suppressing Half B legitimate extensions does not count as improvement. If the gold itself does not exercise two of three axes, the gold is under-authored — this is a Stage C regression and C.G4 should have caught it. Stop and escalate.
...
4. Run cross-episode mode. "Cross-episode mode" means: invoke `/build_assistive_package` on episode 4 while episode 3's already-generated package is present on disk in `artifacts/overton-park/episodes/episode_03/`. The analyst and prose agents consume episode 3's `ground_truth.yaml` and `prose.yaml` via the optional sibling-episode inputs declared in D.1 and D.3, so that (a) `connects_to` references in episode 4 resolve against concrete prior content, and (b) `pedagogical_register` can be checked for drift against the prior episode. Because the optional inputs are declared in Stage D, no new orchestration and no D regression should be required here. If cross-episode mode nevertheless surfaces a prompt gap, it is a D regression — fixed in the relevant D substage, not patched in E.
5. **Cross-story smell test (anti-overfit, non-gated).** After E.3.G4 passes, run `/build_assistive_package` on one passage of one *other* story — `saving-the-maker-space` episode 1 passage 1 is the default; any passage from any non-Overton-Park story works. No gold is authored; no gate is attached. The operator reads the output against the plan's exit-criteria rubrics and asks, for each of the four agent outputs: does this look plausible for *this* story, or does it look like Overton Park transplanted? Specifically: does the analyst identify facets actually evidenced in this passage (not forced Overton-Park-shaped findings); do the diagnostic entries sound like *this* story's students; does `episode_opening` match *this* story's voice; do the discussion cues honor *this* story's cast without referencing characters or dynamics that don't exist here? Write a one-paragraph smell-test report appended to the inventory with one of three outcomes: **looks fine** (proceed to Stage F), **slightly off but not structural** (note observations, proceed to F, watch for confirmation at F.1), or **looks broken** (stop; return to the relevant D substage; see Rollback below). The smell test is informational by design: gating it would require subjective criteria without a gold to anchor them. Its purpose is to catch coarse overfit while D is still warm and cheap to revisit, rather than discovering it at F.1 where rollback is expensive.
...
If the cross-story smell test (task 5) returns "looks broken," it does not trigger a gate failure — Stage E has technically exited — but it does trigger a deliberate return to the relevant D substage before Stage F begins. The operator identifies which agent's output looked Overton-Park-shaped, reopens that D substage, and revises the prompt with explicit guidance to avoid assuming Overton Park's cast, register, or dynamics. After the D revision, E.3 re-runs (including the smell test on a different passage of the same non-Overton-Park story to confirm the fix generalized). This path is cheaper than discovering the same overfit at F.1, which is why the smell test exists at E.3 exit rather than being folded into F.1.
...
- `framework/docs/pipeline-revision-implementation.md` — this file — moved to `archive/pre-revision-2026-04-09/pipeline-revision-implementation.md` with a `frozen YYYY-MM-DD` header (the actual date filled in at retirement time).
...
**Gates.**
- **F.2.G1 (mechanical).** Run F.2.G1 *after* task 7 (this file has been moved to the archive). Run the grep in two passes. Pass 1 — live code surface: `grep -r "evaluator\|analysis_reviewer\|scaffolding_reviewer\|analyze_transcript\|design_scaffolding\|configure_session\|scaffolding\.yaml\|session\.yaml\|pipeline-spec\.md" framework/pipeline/ framework/schemas/ framework/reference/ apps/` with `--exclude-dir=archive` returns zero matches. Pass 2 — docs surface: the same grep under `framework/docs/` is allowed to match *only* in lines tagged with a sentinel comment `<!-- legacy-history -->` or inside fenced blocks marked `legacy-history`; unmarked matches fail the gate. Before running the gate, the operator adds the sentinel to the pipeline-revision spec/memo documents (`pipeline-revision-plan.md`, `pipeline-architecture.md`) on every line that mentions the retired surface as history. Rationale: excluding by filename is fragile under rename and does not scale as new docs reference history; the sentinel is explicit and greppable.
...
- **F.2.G4 (mechanical — regression).** After retirement, re-run the merge script and the package reviewer against the *cached Stage E agent outputs* (the four `{ground_truth,diagnostic,prose,discussion}.yaml` files from the pre-retirement E.3 and F.1 runs, held fixed on disk). The merge script's deterministic derivations must produce a post-retirement `assistive_package.yaml` whose `yaml.safe_load` equals the pre-retirement one (ignoring key order and whitespace); the package reviewer must return ACCEPT on both. This isolates the regression check to the deterministic surface that retirement could actually break (merge script, reviewer, schema loading, bootstrap sync) — re-running LLM agents would introduce nondeterminism unrelated to the gate's purpose. Timestamps, run IDs, and other fields known to vary are excluded by name, with a diff report appended to the inventory. Additionally, re-run `/build_assistive_package` end-to-end on Overton Park episode 3 *after* retirement and verify the output passes the same mechanical gates as E.3 (schema validity, enforcement checks, reviewer ACCEPT) — this is a liveness check, not a byte-equality check.
