...
- `lens_visibility` — per-lens, two orthogonal enum fields plus a prose description. `engagement` (`none | partial | high`) is a pure observation about the transcript: how much the characters actually used the lens. `affordance` (`none | thin | moderate | rich`) is a judgment about the passage: how much the topic gives the lens to work with if it were engaged fully. `what_shows` is a short prose description of how the lens manifests (or fails to) in this passage. The two enum fields together drive the diagnostic agent's per-lens response-space population rules (see §2.3) and the discussion agent's role card rule (see §2.5). Rationale: a single collapsed signal value conflated two distinct judgments — "did the characters engage" and "could they have engaged more given the topic" — which in turn made the weak-signal case ambiguous between "characters had a reasoning gap" and "topic naturally quiet." Splitting the fields makes each judgment auditable on its own terms, eliminates the need for a separate `weak_because` qualifier, and folds the `lens_not_applicable` escape hatch cleanly into `affordance: none`. Invalid combinations: `(engagement: partial, affordance: none)` and `(engagement: high, affordance: none)` — the analyst cannot engage with a lens the topic affords nothing; the merge script rejects these combinations.
- `turn_annotations` — per-turn inverse index: `speaker`, `turn_id`, `moves[]`, `facet_signals[]` (with polarity and strength), `why_it_matters`, and `discussion_cue_seeds[]`. The seeds are a shallow enumeration of creative directions this turn could support (e.g., `[source_credibility, authority_deference, counterfactual_what_if]`) — raw material for the discussion agent, not student-facing prose. **Granularity policy (resolves Open Question 7.3):** every turn inside the passage's `turn_range` gets an entry, one-to-one with the transcript (array length equals transcript turn count within the range). Every entry carries `speaker` and `turn_id`. The remaining content fields (`moves[]`, `facet_signals[]`, `why_it_matters`, `discussion_cue_seeds[]`) are populated **iff the turn is load-bearing**, where load-bearing is defined framework-relatively: a turn is load-bearing iff at least one of (a) it exhibits a facet signal, (b) it exhibits a lens transition contributing to `perspective_transitions[]`, (c) it reveals a cognitive pattern or social dynamic operating on the discussion (feeding `causal_layer`), or (d) it makes a claim that later turns respond to with framework-visible content. Otherwise the entry is written with `speaker` and `turn_id` populated and all content fields empty. An empty entry is a **positive assertion** that the framework has nothing to say about the turn, not a mark of analyst omission; it is auditable and a reviewer may dispute any empty turn as a legitimate extension.
...
- `response_space.by_lens.{logic, evidence, scope}` — per lens, four categories of reading. **Every entry across all four categories carries a `next_move` field** — a single-sentence prompt the app delivers when a student's actual writing pattern-matches to that entry.
  ...
  **Per-lens population rules — the engagement/affordance matrix.** The diagnostic agent populates each lens's four categories mechanically from the lens's `engagement` and `affordance` values in `ground_truth.lens_visibility` (§2.1). The rules are:

  - `likely_readings[]` non-empty iff `engagement ∈ {partial, high}` AND `affordance ≠ none`. Rationale: a likely reading predicts students will imitate the characters' engagement; if the characters didn't engage, there's nothing to imitate.
  - `partial_readings[]` non-empty iff `engagement: partial` AND `affordance ≠ none`. Rationale: a partial reading is specifically the "student half-reaches for this lens" prediction, and the ground is the characters' own partial engagement. High engagement produces completed readings, not partial ones.
  - `misreadings[]` non-empty iff `engagement ∈ {partial, high}` AND `affordance ≠ none`. Rationale: for students to mis-apply the lens, the transcript has to model at least some attempt.
  - `blindspots[]` non-empty iff `affordance ∈ {moderate, rich}` OR (`affordance: thin` AND `engagement ∈ {none, partial}`). Rationale: a blindspot is "student will miss what this lens would reveal." It requires both that the lens has something to reveal (`affordance ≠ none`) and that the characters didn't fully exhaust what was there (`engagement ≠ high`). The `(engagement: high, affordance: thin)` cell is the one exception where blindspots are empty by design — the characters exhausted the thin lens, so there's no specific missed observation to name. The meta-observation that the lens is at its limits in this passage is carried by the analyst's `what_shows` prose, not by a structured blindspot entry.
  - All four categories empty iff `affordance: none`. This is the escape hatch, folded into the normal field: if the topic affords the lens nothing (e.g., pure character backstory with no decision under discussion), the diagnostic has nothing to say about the lens. The merge script requires `engagement: none` whenever `affordance: none` — other combinations are invalid.

  **Every `blindspots[]` entry** carries `text`, `next_move`, `recommended_lens_switch`, `description`, and `why_predictable`. The `description` names what students will miss; the `why_predictable` grounds the prediction in specific features of the transcript.

  **The matrix cells and their urgency.** Five cells have pedagogically distinct urgency that the diagnostic's blindspot framing should reflect:

  | engagement | affordance | blindspot urgency |
  |---|---|---|
  | none | rich | **highest** — characters had a gap; reasoning failure; students will inherit it |
  | none | moderate | high — characters missed a meaningful opportunity |
  | none | thin | low — topic naturally quiet; habit-building framing |
  | partial | rich/moderate/thin | moderate — blindspot names what the partial engagement stopped short of |
  | high | rich/moderate | lower — characters engaged fully; blindspot names subtler edges |

  The `(engagement: none, affordance: rich)` case is the framework's most pedagogically valuable blindspot — the passage looks rhetorically complete (other lenses are active) but the missing lens is the teaching moment. Skipping or hand-waving this case drops the framework's distinctive lens-switching move.

  **Role purity.** Both `engagement` and `affordance` are analyst judgments about the transcript and the topic. Neither is a prediction about students. The diagnostic's response-space categories predict students, but those predictions are *grounded* in the analyst's observations via the matrix — the diagnostic never re-observes the transcript, it consumes the analyst's structured observations as inputs.
...
- `role_cards[]` — per passage, one card per lens (up to 3) **for every lens where `affordance ∈ {moderate, rich}`**, each with a one-sentence stance grounded in that lens's `what_shows`, plus a sentence stem ("From a Logic view, I think ___ because ___"). The app deals these to students so groups start with **guaranteed disagreement**. This is the single highest-leverage addition for "minimizing stuck groups." **Role card rule keys off affordance, not engagement.** A lens with `affordance: rich` but `engagement: none` (the donation-passage Scope case) makes an excellent role card because it gives a student a lens the characters didn't use — maximum generative tension. Gating role cards on engagement would drop exactly the cases where the role card is most pedagogically valuable. Lenses with `affordance: thin` are excluded because the role card student would have thin grounds for argument; their content is carried by the diagnostic's blindspot entries instead. Lenses with `affordance: none` are excluded trivially.
...
- Every `attention_cues` rung's target lens corresponds to a lens with `affordance ∈ {moderate, rich}` in `lens_visibility`.
...
- **Response-space category completeness** per lens, enforced mechanically against the §2.3 engagement/affordance matrix. For every lens, read `engagement` and `affordance` from `ground_truth.lens_visibility.{lens}` and check: (a) `(engagement: partial, affordance: none)` and `(engagement: high, affordance: none)` are invalid combinations — exit nonzero; (b) `likely_readings[]` non-empty iff `engagement ∈ {partial, high}` AND `affordance ≠ none`; (c) `partial_readings[]` non-empty iff `engagement: partial` AND `affordance ≠ none`; (d) `misreadings[]` non-empty iff `engagement ∈ {partial, high}` AND `affordance ≠ none`; (e) `blindspots[]` non-empty iff `affordance ∈ {moderate, rich}` OR (`affordance: thin` AND `engagement ∈ {none, partial}`); (f) all four categories empty iff `affordance: none`; (g) every `blindspots[]` entry carries `text`, `next_move`, `recommended_lens_switch`, `description`, and `why_predictable`. Exit nonzero on first failure; the error message names the lens, the (engagement, affordance) pair, and the specific rule violated.
...
- **`role_cards[]` lens validity** — each card's lens has `affordance ∈ {moderate, rich}`.
...
**Resolved (Open Question 7.3 — turn annotation granularity).** Policy: every turn inside the passage's `turn_range` gets an entry; `speaker` and `turn_id` are always populated; content fields are populated iff the turn is load-bearing in the framework-relative sense defined in §2.1. This preserves positional indexing for turn-level UI affordances (Rule 6), keeps authoring cost proportional to real signal density, and makes empty entries auditable as positive assertions rather than omissions. The pre-Stage-1 spike is reduced from policy selection to policy calibration: hand-annotate Overton Park episode 3's densest passage under the agreed policy, record token counts, and log borderline turns as worked examples for the analyst prompt.
...
**Exit criterion.** Every load-bearing turn meets the mechanical cue-count floor (≥ distinct angles). Sampled cues exercise at least two of the three axes across the episode. Role cards are produced per lens where `affordance ∈ {moderate, rich}`. Persona-projected cues honor established character voices.
...
- ~~**Weak-signal lens policy.**~~ **Resolved** — see §2.1 `lens_visibility` (replaced three-value `signal` with orthogonal `engagement` and `affordance` fields), §2.3 engagement/affordance population matrix, and §2.6 response-space category completeness check. The weak-signal case is now one column of the matrix; category population rules apply uniformly across all cells, with the escape hatch folded into `affordance: none`.
