...
**Source A — cross-lens visibility.** Operationalized analytically by `lens_visibility` (per-lens signal strength and content) and `perspective_transitions` (directional pairs between lenses describing what one lens sees that another misses on this specific passage). Perspective transitions are required on every passage. The LLM pipeline is uniquely positioned to produce them — reasoning about "what does Logic see that Evidence misses here" requires content-specific analysis that a non-AI app cannot do at runtime.
...
**Cognitive job.** *Hold a student error model in mind and write the calibrated responses to foreseeable stuck states.* Imagine what 6th-grade students will get wrong and how to recover them. Produce the response space per lens under the per-lens population rules in `pipeline-revision-plan.md` §2.3 — every lens with `signal ∈ {moderate, strong}` gets the full four-category treatment, every lens with `signal: weak` gets empty `likely_readings`/`partial_readings`/`misreadings` and **required, non-empty `blindspots`** with grounded `description` and `why_predictable` fields. Write the graduated hint ladder with position tags and `recommended_lens_switch` on blindspots. Calibrate struggle metadata. Describe stall signals with silence-breakers below the hint ladder. Anticipate class-level divergence and write the `classroom_move` on every entry. Populate conditional blocks if the story opted into them.

**Weak-signal lenses are first-class work, not edge cases.** The diagnostic agent produces *engagement predictions* (likely/partial/mis-readings) for lenses students are using, and *blindspot predictions* for lenses students are missing. Both are first-class outputs. The weak-signal-lens blindspot is often the passage's most pedagogically valuable content, because it is the entry point for the framework's distinctive lens-switching move — getting students to notice and try a lens the transcript didn't model for them. A passage may look rhetorically complete with two strong lenses and still have a weak-signal third lens whose blindspot is the passage's most important teaching moment; rhetorical completeness masks the missing lens, and the required-blindspot rule forces the diagnostic to confront it. Skipping or hand-waving a weak-signal-lens blindspot is a more serious failure than a generic entry on an active-lens category, because there is no other content in that lens to compensate.
...
5. **`next_move` quality** — sampled entries act on the reading they attach to (push on `what_they_miss` for partial readings, redirect for misreadings, deepen for likely readings). Generic moves that would work on any reading are flagged. **Weak-signal-lens blindspots are held to a higher bar:** when a lens has `signal: weak`, its blindspot entries are the only pedagogical content in that lens, so `recommended_lens_switch` and `next_move` carry full pedagogical weight. A generic or non-specific entry on a weak-signal-lens blindspot counts as a larger failure than a generic entry on an active-lens category, because there is no other content in that lens to compensate. Every weak-signal-lens blindspot must also carry grounded `description` and `why_predictable` fields that reference specific features of the transcript the prediction rests on.
...
A story that lightly populates instructional metadata (one-level hint ladder, `cognitive_only` interactions where permitted, empty `blindspots` when a lens is weak-signal, few discussion cues when a turn is thin) is not flagged as under-specified. The reviewer checks that what is there is well-formed, not that the full machinery is exercised.
