...
5. Commit the schemas and the gold files in the same commit. The commit message names Overton Park episode 3 as the gold baseline. **Weak-signal-lens authoring constraint (spec §2.3, memo §3.2):** the gold `diagnostic.yaml` must include at least one passage where some lens is weak-signal *and* `lens_not_applicable != true`, so the gold exercises the required-blindspot path with full population (`description`, `why_predictable`, `recommended_lens_switch`, `next_move`). If Overton Park episode 3 does not contain such a passage when the analyst surveys its `lens_visibility`, the operator either (a) selects a different episode that does, updating the baseline declaration in this task and in Stage D, or (b) records the absence in the inventory as a Stage C gap and authors an additional minimal passage from another story to cover the weak-signal case. This constraint ensures the gold demonstrates the policy and D.2.G1's weak-signal enforcement check has gold coverage to validate against.
...
**Gates.**
- **D.2.G1 (mechanical).** `validate_schema.py` passes. `next_move` is a non-empty string on every `response_space` entry. `recommended_lens_switch` populated on every `blindspots[]` entry. **Weak-signal-lens population rules enforced (spec §2.3, §2.6):** for every lens in the generated diagnostic where `ground_truth.lens_visibility.signal` is `weak` and `lens_not_applicable != true`, `likely_readings[]`, `partial_readings[]`, and `misreadings[]` are empty and `blindspots[]` is non-empty with every entry carrying `text`, `next_move`, `recommended_lens_switch`, `description`, and `why_predictable`. For every lens with `lens_not_applicable: true`, all four categories are empty and `not_applicable_reason` is non-empty.
- **D.2.G2 — Half A (agent review — diagnostic specificity; correctness floor; from spec §5 Stage 3 gate review; BLOCKING).** Sample five entries per category (likely_readings, partial_readings, misreadings, blindspots) across the three lenses. Mark each as "passage-specific" (names something in the actual transcript) or "generic" (could apply to any passage). ≥80% passage-specific to pass. Generic entries are unambiguously the agent's fault. **Weak-signal-lens blindspots are weighted higher in the sample:** a generic or non-specific entry on a weak-signal-lens blindspot counts as a larger failure than a generic entry on an active-lens category (memo §3.5 criterion 5 clarification), because there is no other content in that lens to compensate. The sample must include at least two weak-signal-lens blindspots when the gold contains any.
