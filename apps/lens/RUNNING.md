...
The Lens pipeline turns one per-episode draft into a complete student- and teacher-facing artifact set for one episode of one story. Five commands across five stages. Each command runs autonomously between operator touchpoints — the operator's authoring work happens in Phase 6 (writing the story design doc and per-episode drafts as prose); the pipeline (Phase 7) is mechanical execution of those drafts.

This guide is for **operators running the pipeline**. For the prose-first authoring loop (Phase 6) and the per-episode draft template, see `framework/docs/operator-manual.md` and Appendix B of `framework/docs/story-pipeline-revision.md`.
...
**One conversation per command.** Run each slash command in a fresh conversation. Token cost is lower, failure isolation is cleaner, and the autonomous reviewer loops within each command don't need conversational context to do their job.
...
| 4 | `/design_scaffolding <story_id> <episode_number>` |
| 5 | `/configure_session <story_id> <episode_number>` |
...
The draft lives at `framework/docs/stories/{story_id}/episode_{NN}.md`. It is authored in Phase 6 as prose with YAML frontmatter — see Appendix B of `framework/docs/story-pipeline-revision.md`. If it does not exist, you are not ready to run the pipeline; return to Phase 6.
...
This walks the story design doc and every per-episode draft in the story directory and runs all the cross-episode rules (lens distribution, mixed-valence rotation, strength/weakness rotation, coverage closure). Halt and return to Phase 6 if it reports any FAIL.
...
- **Restart upstream** — if the failure indicates a structural problem, return to `/create_episode`. If the same signal fails to land across two episodes, return to Phase 6 and revise the per-episode draft (or, for character-level drift, the story design doc).
...
The shared artifacts (`episode.yaml`, `transcript.yaml`, `analysis.yaml`) are reusable by other applications. Reasoning Lab can run on the same episode by initializing for that app and running its own stages — the `lens/` and `reasoning-lab/` subdirectories coexist under the same episode directory.
