...
function isStageRouteAllowed(currentStage: SessionRouteStage, requestedStage: SessionRouteStage): boolean {
  if (currentStage === requestedStage) {
    return true;
  }

  if (currentStage === "landing" && requestedStage === "read") {
    return true;
  }
...
export function SessionStagePage({
  ...
  useEffect(() => {
    ...
    if (!isStageRouteAllowed(nextStage, stage)) {
