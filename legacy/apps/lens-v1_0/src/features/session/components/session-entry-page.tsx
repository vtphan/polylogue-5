...
import { SESSION_INDEX_KEY } from "@/lib/storage/keys";
...
let cachedSessionIndexRaw: string | null = null;
let cachedSessionIndexSnapshot: SessionIndexEntry[] = EMPTY_SESSION_INDEX;

function subscribeToSessionIndex(): () => void {
  return () => {};
}

function getSessionIndexSnapshot(): SessionIndexEntry[] {
  if (typeof window === "undefined") {
    return EMPTY_SESSION_INDEX;
  }

  const rawValue = window.localStorage.getItem(SESSION_INDEX_KEY);
  if (rawValue === cachedSessionIndexRaw) {
    return cachedSessionIndexSnapshot;
  }

  cachedSessionIndexRaw = rawValue;
  cachedSessionIndexSnapshot = loadSessionIndex();
  return cachedSessionIndexSnapshot;
}
