...
- dialog that later stages can work with
...
Do not read `reference/flaw-taxonomy.yaml`. The story designer works at the story level in plain voice; downstream pipeline stages handle anything taxonomy-shaped.
...
- hint at the kinds of thinking moves the story will practice, in plain student-facing language
...
The story should work in a literary sense before it is used for instruction. Characters are people with voices, not instructional roles.

### 2. Keep the Artifact Practical
...
- the episode set is roughly mapped and the story-level arc reads coherently
